...
```{r}
raw_1980 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
unpacked_1980 <- httr::content(raw_1980, as = "text", encoding = "UTF-8") 
final_1980 <- jsonlite::fromJSON(unpacked_1980)

raw_1999 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
unpacked_1999 <- httr::content(raw_1999, as = "text", encoding = "UTF-8") 
final_1999 <- jsonlite::fromJSON(unpacked_1999)

raw_2018 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
unpacked_2018 <- httr::content(raw_2018, as = "text", encoding = "UTF-8") 
final_2018<- jsonlite::fromJSON(unpacked_2018)
```
...
```{r}
raw_data2 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
unpacked_data2 <- httr::content(raw_data2, as = "text", encoding = "UTF-8") 
final_data2<- jsonlite::fromJSON(unpacked_data2)
```
...
In the above function, our first object, `raw_data`, now changes based on how we define our year argument. We accomplish this through `paste0()`, which takes listed objects, transforms them into characters (if they aren't already), and concatenates them into a single character string. 

paste () — this function concatenates the input values and returns a single character string.
paste0 () — this function concatenates the input values in a single character string. 
The difference between: paste and paste0 is that paste function provides a separator operator, whereas paste0 does not.
```{r}
my_sentence <- "I need at least"
my_other_sentence <- "pints of ice cream a day"

paste(my_sentence, " ", 08, " ", my_other_sentence, "!")
...
```{r}
unit_visitation <- function(park_code, startyear, endyear){
  # raw <- httr::GET(url =  paste0("https://irmaservices.nps.gov/v3/rest/stats/total/", year))
  raw3<- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=" , park_code, "&startMonth=1&startYear=", startyear, "&endMonth=12&endYear=", endyear))
  unpacked3 <- httr::content(raw3, as = "text", encoding = "UTF-8") 
  final3<- jsonlite::fromJSON(unpacked3)
  return(final3)
  }
```
...
```{r}
pull_rocky <- unit_visitation(park_code= "ROMO",startyear = 1990, endyear=2021)
pull_everglades<- unit_visitation(park_code= "EVER",startyear = 1990, endyear=2021)
pull_theodore<- unit_visitation(park_code= "THRO",startyear = 1990, endyear=2021)
```
...
```{r}
unit_visitation <- function(park_code, startyear= 1980, endyear=2021){
  # raw <- httr::GET(url =  paste0("https://irmaservices.nps.gov/v3/rest/stats/total/", year))
  raw3<- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=" , park_code, "&startMonth=1&startYear=", startyear, "&endMonth=12&endYear=", endyear))
  unpacked3 <- httr::content(raw3, as = "text", encoding = "UTF-8") 
  final3<- jsonlite::fromJSON(unpacked3)
...
pull_rocky2 <- unit_visitation(park_code= "ROMO")
pull_everglades2<- unit_visitation(park_code= "EVER")
pull_theodore2<- unit_visitation(park_code= "THRO")
```
...
```{r}
#creating vector with codes of interest
park_codes<- c('ROMO','EVER',"THRO")

# empty list to store the data
parks_floop <- vector("list", length = length(park_codes))

#build for loop
for (i in 1:length(park_codes)) {
  parks_floop[[i]] <- unit_visitation(park_codes[i])
}
```
...
#whatever comes after ~ needs to be performed on years
```
...
```{r}
#park_code is any one individual code, park_codes is the list we built
parks_map<-park_codes %>%
  map(~ unit_visitation(park_code= . ))

#putting into a single dataframe
parks_multi_years <- bind_rows(parks_map)
```
