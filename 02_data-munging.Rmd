...
```{r}
#parkwide is total visitations, units is per unit how many at an individ park 
...
```{r}
#attempt one, not interactive 
yearly_parks<- yearly %>%
  filter(!(UnitCode== 'Parkwide'))
yearly_parkwide<- yearly %>%
  filter(UnitCode== 'Parkwide')

 a<- ggplot() +
    geom_point(data= yearly_parks, mapping = aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(data= yearly_parks, mapping = aes(x = Year, y = RecVisitation, color = UnitCode)) +
    theme_bw(base_size=10)

 b<- ggplot()+
    geom_point(data= yearly_parkwide, mapping = aes(x = Year, y = RecVisitation)) +
    geom_path(data= yearly_parkwide, mapping = aes(x = Year, y = RecVisitation)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    theme_bw(base_size=10)
...
plotly::ggplotly(
  ggplot(data=yearly) +
    ...
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(~UnitCode !='Parkwide', scales = "free_y", nrow=2) +
    theme_bw(base_size=10)
)
```
...
## Pivoting
**changing the structure/layout of the dataframe**
...
narrow_data <- wide_data %>%
  pivot_longer(cols = -Year, #columns you dont want to be long
               names_to = "Park",
...
```{r}
park_percentage<- wide_data %>%
  pivot_longer(cols= -c(Parkwide,Year),
               names_to = "POI",
               values_to = "RecVisitation") %>%
  mutate(percent= (RecVisitation/Parkwide)*100)
  
plotly::ggplotly(
  ggplot(park_percentage)+
    geom_col(aes(x=Year, y=percent, color= POI))
)
```
...
```{r}
##scale is off
joined_park_percentage<- joined_data %>%
  group_by(Year) %>%
  mutate(percent= (RecreationVisitors.x/RecreationVisitors.y)*10)

plotly::ggplotly(
  ggplot(joined_park_percentage)+
    geom_col(aes(x=Year, y=percent, color= UnitCode.x))
)
```
...
```{r}
#first attempt
park_percentage<-park_percentage %>%
  group_by(POI) %>%
  mutate(avg= mean(RecVisitation)) %>%
  ungroup(POI) %>%
  mutate(rank= min_rank(avg))
```

```{r}
#trying to change the structure more
#use mutate_at!!
#park_rank<- wide_data %>%
#  mutate_at(.vars = parks, .funs= ~(.mean)) 
...
park_rank<- wide_data %>%
  ...
  mutate(rank=(min_rank(desc(Average_Visitation))))

```

GRSM had the most visitors with 9907814 from 1990-2021.
ACAD had the least visitors with 2636266 from 1990-2021.
